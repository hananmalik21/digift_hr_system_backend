/**
 * Holiday View
 * Handles response formatting for HOLIDAYS endpoints
 */
...
export function sendHolidayList(res, req, holidays, meta = {}) {
  ...
  const responseMeta = {
    ...generateBaseMetadata(req, {
      ...
      total: meta.total !== undefined ? meta.total : holidays.length,
  ...
  if (meta.pagination) {
    responseMeta.pagination = {
      ...
      page_size: meta.pagination.pageSize || holidays.length,
      total: meta.total !== undefined ? meta.total : holidays.length,
  ...
  const convertedData = convertKeysToSnakeCase(holidays);
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object|null} holiday - Holiday object or null
 */
export function sendHoliday(res, req, holiday) {
  if (!holiday) {
  ...
  const convertedHoliday = convertKeysToSnakeCase(holiday);
  
  res.json({
    success: true,
    meta: generateBaseMetadata(req, {
      execution_time: `${executionTime}ms`,
      holiday_id: convertedHoliday.holiday_id || holiday.HOLIDAY_ID
    }),
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object} holiday - Created holiday
 */
export function sendCreated(res, req, holiday) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object} holiday - Updated holiday
 */
export function sendUpdated(res, req, holiday) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
  ...
  res.json({
    ...
    meta: generateBaseMetadata(req, {
      ...
      holiday_id: convertedHoliday.holiday_id || holiday.HOLIDAY_ID,
      action: 'updated',
      last_updated: convertedHoliday.last_update_date || holiday.LAST_UPDATE_DATE
    }),
    data: convertedHoliday
  });
}
