...
class AbsLookupModel {
  ...
  /**
   * Get all lookups for a tenant
   * @param {number} tenantId - Tenant ID (required)
   * @returns {Promise<Array>} Array of lookups ordered by lookup_code
   */
  ...
  /**
   * Check if lookup_code already exists for a tenant
   * @param {string} lookupCode - Lookup code
   * @param {number} tenantId - Tenant ID
   * @param {number} excludeLookupId - Optional lookup ID to exclude from check (for updates)
   ...
   * @returns {Promise<Object>} Created lookup
   */
  static async create(data, userId) {
    try {
      return await this.executeWithTransaction(async (connection) => {
        ...
        // Check if lookup_code already exists for this tenant
        ...
        // Get next sequence value for LOOKUP_ID
  ...
  /**
   * Update lookup (only lookup_name and status)
   * @param {number} lookupId - Lookup ID
   * @param {number} tenantId - Tenant ID (required for multi-tenant security)
   * @param {Object} data - Updated data (only LOOKUP_NAME and STATUS allowed)
   * @param {string} userId - User ID for audit fields
   * @returns {Promise<Object>} Updated lookup
   */
  static async update(lookupId, tenantId, data, userId) {
    try {
      return await this.executeWithTransaction(async (connection) => {
        ...
        // Verify lookup exists and belongs to tenant
        ...
        // Only allow updating LOOKUP_NAME and STATUS
        ...
        // Fetch and return the updated record using same connection (so we see our own uncommitted update)
        const selectQuery = `SELECT 
          LOOKUP_ID,
          LOOKUP_CODE,
          LOOKUP_NAME,
  ...
  /**
   * Check if lookup has child records in ABS_LOOKUP_VALUES
   * @param {number} lookupId - Lookup ID
   * @param {number} tenantId - Tenant ID (required for multi-tenant security)
  ...
  /**
   * Delete a lookup (only if no child records exist)
   * @param {number} lookupId - Lookup ID
   * @param {number} tenantId - Tenant ID (required for multi-tenant security)
