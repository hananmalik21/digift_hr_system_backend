...
import EntLookupTypeModel from '../model/entLookupTypeModel.js';
import {
  sendLookupTypeList,
  sendLookupType,
  sendCreated,
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await EntLookupTypeModel.findAll(filters);
...
router.get('/:guid', async (req, res) => {
  try {
    ...
    const lookupType = await EntLookupTypeModel.findByGuid(guidHex32);
    if (!lookupType) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const created = await EntLookupTypeModel.create(normalizedData, userId);
...
router.put('/:guid', async (req, res) => {
  try {
    ...
    const existing = await EntLookupTypeModel.findByGuid(guidHex32);
    if (!existing) {
      return sendNotFound(res, req, 'Lookup type not found');
    }
    const normalizedBody = normalizeRequestBody(req.body);
    ...
    const userId = getUserId(req);
    const updated = await EntLookupTypeModel.updateByGuid(guidHex32, normalizedData, userId);
...
router.delete('/:guid', async (req, res) => {
  try {
    ...
    await EntLookupTypeModel.deleteByGuid(guidHex32);
