/**
 * Rec Lookup Value View
 * Handles response formatting for REC_LOOKUP_VALUES endpoints
 */
...
export function sendServerError(res, req, message, error = null) {
  ...
  if (error) {
    console.error('Server error in rec lookup values:', error);
    details = { message: error.message, code: error.code, errorNum: error.errorNum };
