...
import RecLookupTypeModel from '../model/recLookupTypeModel.js';
import {
  sendLookupTypeList,
  sendLookupType,
  sendCreated,
  ...
  sendNotFound,
  sendConflict
} from '../view/recLookupTypeView.js';
...
function parsePagination(query) {
  ...
  if (query.page !== undefined) {
    ...
    if (isNaN(parsedPage) || parsedPage < 1) throw new Error('Invalid page number. Must be a positive integer.');
    page = parsedPage;
  }
  if (query.page_size !== undefined) {
    const parsedPageSize = parseInt(query.page_size);
    if (isNaN(parsedPageSize) || parsedPageSize < 1) throw new Error('Invalid page_size. Must be a positive integer.');
...
function validateLookupTypeData(data, isUpdate = false) {
  ...
  if (!isUpdate) {
    if (!data.TYPE_CODE || (typeof data.TYPE_CODE === 'string' && data.TYPE_CODE.trim() === '')) errors.push('TYPE_CODE is required');
    if (!data.TYPE_NAME || (typeof data.TYPE_NAME === 'string' && data.TYPE_NAME.trim() === '')) errors.push('TYPE_NAME is required');
  } else {
    if (data.TYPE_CODE !== undefined && (typeof data.TYPE_CODE !== 'string' || data.TYPE_CODE.trim() === '')) errors.push('TYPE_CODE cannot be empty');
    if (data.TYPE_NAME !== undefined && (typeof data.TYPE_NAME !== 'string' || data.TYPE_NAME.trim() === '')) errors.push('TYPE_NAME cannot be empty');
  }
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await RecLookupTypeModel.findAll(filters);
    const { lookupTypes, total } = result;
    const paginationMeta = buildPaginationMeta(filters.pagination.page, filters.pagination.pageSize, total);
    sendLookupTypeList(res, req, lookupTypes, { total, pagination: paginationMeta });
  } catch (error) {
...
router.get('/:guid', async (req, res) => {
  try {
    ...
    const lookupType = await RecLookupTypeModel.findByGuid(guidHex32);
    if (!lookupType) return sendNotFound(res, req, 'Lookup type not found');
...
router.post('/', async (req, res) => {
  try {
    ...
    const created = await RecLookupTypeModel.create(normalizedData, userId);
...
router.put('/:guid', async (req, res) => {
  try {
    ...
    const existing = await RecLookupTypeModel.findByGuid(guidHex32);
    if (!existing) return sendNotFound(res, req, 'Lookup type not found');
    ...
    const userId = getUserId(req);
    const updated = await RecLookupTypeModel.updateByGuid(guidHex32, normalizedData, userId);
...
router.delete('/:guid', async (req, res) => {
  try {
    ...
    await RecLookupTypeModel.deleteByGuid(guidHex32);
