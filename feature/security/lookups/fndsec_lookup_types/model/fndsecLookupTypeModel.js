...
/**
 * FNDSEC lookup types — same visibility rules as COMP lookups use for TENANT_ID:
 * rows with ENTERPRISE_ID NULL are global; scoped rows match the requested enterprise.
 */
class FndsecLookupTypeModel {
  static TABLE_NAME = 'FNDSEC.FNDSEC_LOOKUP_TYPES';
  ...
  static async findAll(filters = {}) {
    try {
      ...
      bindParams.push(enterpriseIdNum);
      paramIndex++;

      if (filters.activeFlag !== undefined) {
        const activeVal =
          filters.activeFlag === true || filters.activeFlag === 'Y' || filters.activeFlag === 1 ? 'Y' : 'N';
  ...
  static async create(data, userId) {
    try {
      return await this.executeWithTransaction(async (connection) => {
        let lookupTypeId;
        try {
          const seqQuery = `SELECT FNDSEC.FNDSEC_LOOKUP_TYPES_SEQ.NEXTVAL AS NEXT_ID FROM DUAL`;
        ...
        const bindParams = [
          ...
          data.ACTIVE_FLAG !== undefined && data.ACTIVE_FLAG !== null
            ? data.ACTIVE_FLAG === true || data.ACTIVE_FLAG === 'Y' || data.ACTIVE_FLAG === 1
              ? 'Y'
              : 'N'
            : 'Y',
          userId || 'SYSTEM',
...
export default FndsecLookupTypeModel;
