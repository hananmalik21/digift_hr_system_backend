...
import FndsecLookupTypeModel from '../model/fndsecLookupTypeModel.js';
import {
  sendLookupTypeList,
  sendLookupType,
  sendCreated,
  ...
  sendNotFound,
  sendConflict
} from '../view/fndsecLookupTypeView.js';
...
router.get('/', async (req, res) => {
  try {
    ...
    const filters = { enterpriseId: enterpriseIdNum };
    ...
    const result = await FndsecLookupTypeModel.findAll(filters);
...
router.get('/:guid', async (req, res) => {
  try {
    ...
    const lookupType = await FndsecLookupTypeModel.findByGuid(guidHex32);
    if (!lookupType) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const created = await FndsecLookupTypeModel.create(normalizedData, userId);
...
router.put('/:guid', async (req, res) => {
  try {
    ...
    const existing = await FndsecLookupTypeModel.findByGuid(guidHex32);
    if (!existing) {
      return sendNotFound(res, req, 'Lookup type not found');
    }
    const normalizedBody = normalizeRequestBody(req.body);
    ...
    const userId = getUserId(req);
    const updated = await FndsecLookupTypeModel.updateByGuid(guidHex32, normalizedData, userId);
...
router.delete('/:guid', async (req, res) => {
  try {
    ...
    await FndsecLookupTypeModel.deleteByGuid(guidHex32);
