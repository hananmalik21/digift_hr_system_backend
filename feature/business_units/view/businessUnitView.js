/**
 * Business Unit View
 * Handles response formatting for BUSINESS_UNIT endpoints
 */
...
/**
 * Send list of business units
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Array} businessUnits - Array of business units
 * @param {Object} meta - Optional metadata (count, filters, pagination, etc.)
 */
export function sendBusinessUnitList(res, req, businessUnits, meta = {}) {
  ...
  const responseMeta = {
    ...generateBaseMetadata(req, {
      ...
      total: meta.total !== undefined ? meta.total : businessUnits.length,
  ...
  if (meta.pagination) {
    responseMeta.pagination = {
      ...
      page_size: meta.pagination.pageSize || businessUnits.length,
      total: meta.total !== undefined ? meta.total : businessUnits.length,
  ...
  const convertedData = convertKeysToSnakeCase(businessUnits);
...
/**
 * Send single business unit
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object|null} businessUnit - Business unit object or null
 */
export function sendBusinessUnit(res, req, businessUnit) {
  if (!businessUnit) {
  ...
  const convertedBusinessUnit = convertKeysToSnakeCase(businessUnit);
  
  res.json({
    success: true,
    meta: generateBaseMetadata(req, {
      execution_time: `${executionTime}ms`,
      business_unit_id: convertedBusinessUnit.business_unit_id || businessUnit.BUSINESS_UNIT_ID
    }),
    data: convertedBusinessUnit
  });
}
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object} businessUnit - Created business unit
 */
export function sendCreated(res, req, businessUnit) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object} businessUnit - Updated business unit
 */
export function sendUpdated(res, req, businessUnit) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
  ...
  res.json({
    ...
    meta: generateBaseMetadata(req, {
      ...
      business_unit_id: convertedBusinessUnit.business_unit_id || businessUnit.BUSINESS_UNIT_ID,
      action: 'updated',
      last_updated: convertedBusinessUnit.last_updated_date || businessUnit.LAST_UPDATED_DATE
    }),
    data: convertedBusinessUnit
  });
}
 ...
 * @param {number} businessUnitId - Deleted business unit ID
 */
export function sendDeleted(res, req, message = 'Business unit deleted successfully', businessUnitId = null) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
  ...
  res.json({
    ...
    meta: generateBaseMetadata(req, {
      execution_time: `${executionTime}ms`,
      business_unit_id: businessUnitId || req.params?.id,
      action: 'deleted'
    })
  });
}
