/**
 * Enterprise View
 * Handles response formatting for ENTERPRISES endpoints
 */
...
/**
 * Send list of enterprises
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Array} enterprises - Array of enterprises
 * @param {Object} meta - Optional metadata (count, filters, pagination, etc.)
 */
export function sendEnterpriseList(res, req, enterprises, meta = {}) {
  ...
  const responseMeta = {
    ...generateBaseMetadata(req, {
      ...
      total: meta.total !== undefined ? meta.total : enterprises.length,
  ...
  if (meta.pagination) {
    responseMeta.pagination = {
      ...
      page_size: meta.pagination.pageSize || enterprises.length,
  ...
  const convertedData = convertKeysToSnakeCase(enterprises);
...
/**
 * Send single enterprise
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object|null} enterprise - Enterprise object or null
 */
export function sendEnterprise(res, req, enterprise) {
  if (!enterprise) {
  ...
  const convertedEnterprise = convertKeysToSnakeCase(enterprise);
  
  res.json({
    success: true,
    meta: generateBaseMetadata(req, {
      execution_time: `${executionTime}ms`,
      enterprise_id: convertedEnterprise.enterprise_id || enterprise.ENTERPRISE_ID
    }),
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object} enterprise - Created enterprise
 */
export function sendCreated(res, req, enterprise) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object} enterprise - Updated enterprise
 */
export function sendUpdated(res, req, enterprise) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
  ...
  res.json({
    ...
    meta: generateBaseMetadata(req, {
      ...
      enterprise_id: convertedEnterprise.enterprise_id || enterprise.ENTERPRISE_ID,
      action: 'updated',
      last_updated: convertedEnterprise.last_updated_date || enterprise.LAST_UPDATED_DATE
    }),
    data: convertedEnterprise
  });
}
 ...
 * @param {number} enterpriseId - Deleted enterprise ID
 */
export function sendDeleted(res, req, message = 'Enterprise deleted successfully', enterpriseId = null) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
  ...
  res.json({
    ...
    meta: generateBaseMetadata(req, {
      execution_time: `${executionTime}ms`,
      enterprise_id: enterpriseId || req.params?.id,
      action: 'deleted'
    })
  });
}
