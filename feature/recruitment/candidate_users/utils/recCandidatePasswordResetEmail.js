import { escapeHtml } from '../../candidates/utils/recCandidateSendEmailContent.js';

const FALLBACK_ENTERPRISE_LABEL = 'Careers';

/**
 * @param {unknown} enterpriseName
 * @returns {string}
 */
export function formatCareersBrandName(enterpriseName) {
  const name = String(enterpriseName ?? '').trim();
  if (!name) return FALLBACK_ENTERPRISE_LABEL;
  if (/careers$/i.test(name)) return name;
  return `${name} Careers`;
}
 ...
 * @returns {string}
 */
export function buildPasswordResetEmailSubject(enterpriseName) {
  return `${formatCareersBrandName(enterpriseName)} — Password Reset Verification`;
}
 ...
 */
export function buildPasswordResetOtpEmailContent(input) {
  const brand = formatCareersBrandName(input.enterpriseName);
  const otp = String(input.otp ?? '').trim();
  const subject = buildPasswordResetEmailSubject(input.enterpriseName);

  const text = [
    `Hello,`,
    '',
    `We received a request to reset the password for your ${brand} candidate account` +
  ...
  const html = [
    ...
    `<p style="margin: 0 0 16px; font-size: 15px;">We received a request to reset the password for your <strong>${safeBrand}</strong> candidate account` +
