/**
 * ABS Leave Policy View
 * Handles response formatting for ABS_LEAVE_POLICIES endpoints
 */
...
/**
 * Send list of leave policies
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Array} policies - Array of leave policies
...
export function sendPolicyList(res, req, policies, meta = {}) {
  ...
  const responseMeta = {
    ...generateBaseMetadata(req, {
      ...
      total: meta.total !== undefined ? meta.total : policies.length,
  ...
  const convertedData = convertKeysToSnakeCase(policies);
...
/**
 * Send single leave policy
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object|null} policy - Leave policy object or null
 */
export function sendPolicy(res, req, policy) {
  ...
  const convertedPolicy = convertKeysToSnakeCase(policy);
  
  res.json({
    success: true,
    meta: generateBaseMetadata(req, {
      execution_time: `${executionTime}ms`,
      policy_id: convertedPolicy.policy_id || policy.POLICY_ID
    }),
    data: convertedPolicy
  });
}
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object} policy - Created leave policy
 */
export function sendCreated(res, req, policy) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
 ...
 * @param {Object} res - Express response object
 * @param {Object} req - Express request object
 * @param {Object} policy - Updated leave policy
 */
export function sendUpdated(res, req, policy) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
  ...
  res.json({
    ...
    meta: generateBaseMetadata(req, {
      ...
      policy_id: convertedPolicy.policy_id || policy.POLICY_ID,
      action: 'updated',
      last_updated: convertedPolicy.last_update_date || policy.LAST_UPDATE_DATE
    }),
    data: convertedPolicy
  });
}
 ...
 * @param {number} policyId - Deleted policy ID
 */
export function sendDeleted(res, req, message = 'Leave policy deleted successfully', policyId = null) {
  const startTime = req._startTime || Date.now();
  const executionTime = Date.now() - startTime;
  ...
  res.json({
    ...
    meta: generateBaseMetadata(req, {
      execution_time: `${executionTime}ms`,
      policy_id: policyId || req.params?.policy_id,
      action: 'deleted'
    })
  });
}
