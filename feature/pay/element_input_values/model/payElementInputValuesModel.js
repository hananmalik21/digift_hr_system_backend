...
const PKG = 'PAY.PAY_ELEMENT_INPUT_VALUES_PKG';
const CREATE_PROC = `${PKG}.CREATE_INPUT_VALUE`;
const UPDATE_PROC = `${PKG}.UPDATE_INPUT_VALUE`;
const DELETE_PROC = `${PKG}.DELETE_INPUT_VALUE`;

const LOG_TAG = 'payElementInputValuesModel';
const GENERIC_ERROR_MESSAGE = 'Unable to process element input value. Please try again.';
...
BEGIN
  ${CREATE_PROC}(
    ...
    P_INPUT_VALUE_ID   => :input_value_id,
    P_INPUT_VALUE_GUID => :input_value_guid,
    P_STATUS           => :status,
    P_MESSAGE          => :message
  );
END;`;
...
function parseCreateOut(outBinds) {
  const ob = outBinds || {};
  const guid = normalizeOutString(ob.input_value_guid);
  return {
    input_value_id: normalizeOutNumber(ob.input_value_id),
    input_value_guid: guid ? normalizeOutGuidHex(guid) : null,
...
async function executePackageMutation(plsql, binds, parseOut) {
  ...
  try {
    ...
    throw new DatabaseError(
      ...
      err,
      resolvePayElementInputValuesUserMessage(null, err)
    );
  } finally {
    try {
 ...
 */
export async function createInputValueViaPackage(payload, createdBy) {
  const binds = {
    payload_json: payloadJsonInBind(payload),
    created_by: auditInBind(createdBy),
    input_value_id: { dir: oracledb.BIND_OUT, type: oracledb.NUMBER },
    input_value_guid: { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 32 },
    ...statusOutBinds()
  };
 ...
 */
export async function updateInputValueViaPackage(inputValueGuidHex, payload, updatedBy) {
  const binds = {
    input_value_guid: guidHexInBind(inputValueGuidHex),
    payload_json: payloadJsonInBind(payload),
 ...
 */
export async function deleteInputValueViaPackage(inputValueGuidHex, deletedBy) {
  const binds = {
    input_value_guid: guidHexInBind(inputValueGuidHex),
