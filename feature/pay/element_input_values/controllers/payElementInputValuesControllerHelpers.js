...
import { firstValidationMessage } from '../validations/payElementInputValues.validation.js';

export const ROUTE_TAG = 'payElementInputValues';
export const FALLBACK_ERROR = 'Unable to process element input value. Please try again.';
...
export function sendNotFoundError(res, message = 'Input value not found') {
...
export function sendSystemError(res, err) {
  if (err instanceof DatabaseError) {
    const message =
      err.userMessage ||
      resolvePayElementInputValuesUserMessage(err.message, err.oracleError) ||
      FALLBACK_ERROR;
...
export async function withPayElementInputValueErrorHandling(res, work) {
  try {
    return await work();
  } catch (err) {
