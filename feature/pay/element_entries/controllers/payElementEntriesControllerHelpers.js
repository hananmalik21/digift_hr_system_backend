...
import { firstValidationMessage } from '../validations/payElementEntries.validation.js';

export const ROUTE_TAG = 'payElementEntries';
export const FALLBACK_ERROR = 'Unable to process element entry. Please try again.';
...
export function sendNotFoundError(res, message = 'Element entry not found') {
...
export function sendSystemError(res, err) {
  if (err instanceof DatabaseError) {
    const message =
      err.userMessage ||
      resolvePayElementEntriesUserMessage(err.message, err.oracleError) ||
      FALLBACK_ERROR;
...
export async function withPayElementEntryErrorHandling(res, work) {
  try {
    return await work();
  } catch (err) {
