...
import { firstValidationMessage } from '../validations/payElementRetroRules.validation.js';

export const ROUTE_TAG = 'payElementRetroRules';
export const FALLBACK_ERROR = 'Unable to process element retro rule. Please try again.';
...
export function sendNotFoundError(res, message = 'Retro rule not found') {
...
export function sendSystemError(res, err) {
  if (err instanceof DatabaseError) {
    const message =
      err.userMessage ||
      resolvePayElementRetroRulesUserMessage(err.message, err.oracleError) ||
      FALLBACK_ERROR;
...
export async function withPayElementRetroRuleErrorHandling(res, work) {
  try {
    return await work();
  } catch (err) {
