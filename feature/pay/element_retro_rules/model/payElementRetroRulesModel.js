...
const PKG = 'PAY.PAY_ELEMENT_RETRO_RULES_PKG';
const CREATE_PROC = `${PKG}.CREATE_RETRO_RULE`;
const UPDATE_PROC = `${PKG}.UPDATE_RETRO_RULE`;
const DELETE_PROC = `${PKG}.DELETE_RETRO_RULE`;

const LOG_TAG = 'payElementRetroRulesModel';
const GENERIC_ERROR_MESSAGE = 'Unable to process element retro rule. Please try again.';
...
BEGIN
  ${CREATE_PROC}(
    ...
    P_RETRO_RULE_ID  => :retro_rule_id,
    P_RETRO_RULE_GUID => :retro_rule_guid,
    P_STATUS         => :status,
    P_MESSAGE        => :message
  );
END;`;
...
function parseCreateOut(outBinds) {
  const ob = outBinds || {};
  const guid = normalizeOutString(ob.retro_rule_guid);
  return {
    retro_rule_id: normalizeOutNumber(ob.retro_rule_id),
    retro_rule_guid: guid ? normalizeOutGuidHex(guid) : null,
...
async function executePackageMutation(plsql, binds, parseOut) {
  ...
  try {
    ...
    throw new DatabaseError(
      ...
      err,
      resolvePayElementRetroRulesUserMessage(null, err)
    );
  } finally {
    try {
 ...
 */
export async function createRetroRuleViaPackage(payload, createdBy) {
  const binds = {
    payload_json: payloadJsonInBind(payload),
    created_by: auditInBind(createdBy),
    retro_rule_id: { dir: oracledb.BIND_OUT, type: oracledb.NUMBER },
    retro_rule_guid: { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 32 },
    ...statusOutBinds()
  };
 ...
 */
export async function updateRetroRuleViaPackage(retroRuleGuidHex, payload, updatedBy) {
  const binds = {
    retro_rule_guid: guidHexInBind(retroRuleGuidHex),
    payload_json: payloadJsonInBind(payload),
 ...
 */
export async function deleteRetroRuleViaPackage(retroRuleGuidHex, deletedBy) {
  const binds = {
    retro_rule_guid: guidHexInBind(retroRuleGuidHex),
