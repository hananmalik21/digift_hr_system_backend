...
const PKG = 'PAY.PAY_ELEMENT_PROCESSING_RULES_PKG';
const CREATE_PROC = `${PKG}.CREATE_PROCESSING_RULE`;
const UPDATE_PROC = `${PKG}.UPDATE_PROCESSING_RULE`;
const DELETE_PROC = `${PKG}.DELETE_PROCESSING_RULE`;

const LOG_TAG = 'payElementProcessingRulesModel';
const GENERIC_ERROR_MESSAGE = 'Unable to process element processing rule. Please try again.';
...
BEGIN
  ${CREATE_PROC}(
    ...
    P_PROCESSING_RULE_ID   => :processing_rule_id,
    P_PROCESSING_RULE_GUID => :processing_rule_guid,
    P_STATUS               => :status,
    P_MESSAGE              => :message
  );
END;`;
...
function parseCreateOut(outBinds) {
  const ob = outBinds || {};
  const guid = normalizeOutString(ob.processing_rule_guid);
  return {
    processing_rule_id: normalizeOutNumber(ob.processing_rule_id),
    processing_rule_guid: guid ? normalizeOutGuidHex(guid) : null,
...
async function executePackageMutation(plsql, binds, parseOut) {
  ...
  try {
    ...
    throw new DatabaseError(
      ...
      err,
      resolvePayElementProcessingRulesUserMessage(null, err)
    );
  } finally {
    try {
 ...
 */
export async function createProcessingRuleViaPackage(payload, createdBy) {
  const binds = {
    payload_json: payloadJsonInBind(payload),
    created_by: auditInBind(createdBy),
    processing_rule_id: { dir: oracledb.BIND_OUT, type: oracledb.NUMBER },
    processing_rule_guid: { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 32 },
    ...statusOutBinds()
  };
 ...
 */
export async function updateProcessingRuleViaPackage(processingRuleGuidHex, payload, updatedBy) {
  const binds = {
    processing_rule_guid: guidHexInBind(processingRuleGuidHex),
    payload_json: payloadJsonInBind(payload),
 ...
 */
export async function deleteProcessingRuleViaPackage(processingRuleGuidHex, deletedBy) {
  const binds = {
    processing_rule_guid: guidHexInBind(processingRuleGuidHex),
