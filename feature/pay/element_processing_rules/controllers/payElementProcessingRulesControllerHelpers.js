...
import { firstValidationMessage } from '../validations/payElementProcessingRules.validation.js';

export const ROUTE_TAG = 'payElementProcessingRules';
export const FALLBACK_ERROR = 'Unable to process element processing rule. Please try again.';
...
export function sendSystemError(res, err) {
  if (err instanceof DatabaseError) {
    const message =
      err.userMessage ||
      resolvePayElementProcessingRulesUserMessage(err.message, err.oracleError) ||
      FALLBACK_ERROR;
...
export async function withPayElementProcessingRuleErrorHandling(res, work) {
  try {
    return await work();
  } catch (err) {
