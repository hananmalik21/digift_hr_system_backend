const SORT_COLUMNS = Object.freeze({
  ...
  processing_type_code: 'PROCESSING_TYPE_CODE',
  priority: 'PRIORITY',
 ...
 */
export function buildPayElementProcessingRuleListWhereClause(filters) {
  ...
  if (filters.processing_type_code != null) {
    whereParts.push('UPPER(v.PROCESSING_TYPE_CODE) = :processing_type_code');
    binds.processing_type_code = String(filters.processing_type_code).trim().toUpperCase();
  }

  if (filters.processing_group_code != null) {
    whereParts.push('UPPER(v.PROCESSING_GROUP_CODE) = :processing_group_code');
    binds.processing_group_code = String(filters.processing_group_code).trim().toUpperCase();
  }
  ...
  if (filters.search != null) {
    whereParts.push(`(
      ...
      OR UPPER(v.PROCESSING_TYPE_CODE) LIKE :search
      OR UPPER(v.PROCESSING_GROUP_CODE) LIKE :search
      OR UPPER(v.LEGISLATIVE_DATA_GROUP) LIKE :search
    )`);
