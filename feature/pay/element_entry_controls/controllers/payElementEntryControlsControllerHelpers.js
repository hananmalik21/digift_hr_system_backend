...
import { firstValidationMessage } from '../validations/payElementEntryControls.validation.js';

export const ROUTE_TAG = 'payElementEntryControls';
export const FALLBACK_ERROR = 'Unable to process element entry controls. Please try again.';
...
export function sendNotFoundError(res, message = 'Entry controls not found') {
...
export function sendSystemError(res, err) {
  if (err instanceof DatabaseError) {
    const message =
      err.userMessage ||
      resolvePayElementEntryControlsUserMessage(err.message, err.oracleError) ||
      FALLBACK_ERROR;
...
export async function withPayElementEntryControlErrorHandling(res, work) {
  try {
    return await work();
  } catch (err) {
