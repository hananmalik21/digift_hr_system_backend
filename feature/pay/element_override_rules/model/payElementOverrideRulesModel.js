...
const PKG = 'PAY.PAY_ELEMENT_OVERRIDE_RULES_PKG';
const CREATE_PROC = `${PKG}.CREATE_OVERRIDE_RULE`;
const UPDATE_PROC = `${PKG}.UPDATE_OVERRIDE_RULE`;
const DELETE_PROC = `${PKG}.DELETE_OVERRIDE_RULE`;

const LOG_TAG = 'payElementOverrideRulesModel';
const GENERIC_ERROR_MESSAGE = 'Unable to process element override rule. Please try again.';
...
BEGIN
  ${CREATE_PROC}(
    ...
    P_OVERRIDE_RULE_ID   => :override_rule_id,
    P_OVERRIDE_RULE_GUID => :override_rule_guid,
    P_STATUS             => :status,
    P_MESSAGE            => :message
  );
END;`;
...
function parseCreateOut(outBinds) {
  const ob = outBinds || {};
  const guid = normalizeOutString(ob.override_rule_guid);
  return {
    override_rule_id: normalizeOutNumber(ob.override_rule_id),
    override_rule_guid: guid ? normalizeOutGuidHex(guid) : null,
...
async function executePackageMutation(plsql, binds, parseOut) {
  ...
  try {
    ...
    throw new DatabaseError(
      ...
      err,
      resolvePayElementOverrideRulesUserMessage(null, err)
    );
  } finally {
    try {
 ...
 */
export async function createOverrideRuleViaPackage(payload, createdBy) {
  const binds = {
    payload_json: payloadJsonInBind(payload),
    created_by: auditInBind(createdBy),
    override_rule_id: { dir: oracledb.BIND_OUT, type: oracledb.NUMBER },
    override_rule_guid: { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 32 },
    ...statusOutBinds()
  };
 ...
 */
export async function updateOverrideRuleViaPackage(overrideRuleGuidHex, payload, updatedBy) {
  const binds = {
    override_rule_guid: guidHexInBind(overrideRuleGuidHex),
    payload_json: payloadJsonInBind(payload),
 ...
 */
export async function deleteOverrideRuleViaPackage(overrideRuleGuidHex, deletedBy) {
  const binds = {
    override_rule_guid: guidHexInBind(overrideRuleGuidHex),
