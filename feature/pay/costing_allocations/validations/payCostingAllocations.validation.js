...
export function parseCostingAllocationGuidParam(raw) {
  return parseGuid(raw, 'costingAllocationGuid');
}
...
function rejectForbiddenFields(errors, body, forbiddenKeys) {
  for (const k of forbiddenKeys) {
    ...
    // If the frontend sends a forbidden field, reject it even when null.
    errors.push(`${k} is not allowed.`);
  }
}

export function validateListCostingAllocationsQuery(query = {}, req) {
  const errors = [];
  ...
  return { enterprise_id, employee_id, status_code, page, limit, offset };
}

export function validateGetCostingAllocationByGuidQuery(query = {}, req) {
  const errors = [];
  const enterprise_id = resolveRequiredEnterpriseIdFromQuery(req, query.enterprise_id, errors);
  throwIfErrors(errors);
  return { enterprise_id };
...
export function validateCreateCostingAllocationBody(body = {}, _req) {
  const errors = [];

  const enterprise_id = parseEnterpriseIdField(errors, body.enterprise_id, { required: true });
  const employee_id = parsePositiveInteger(errors, body.employee_id, 'employee_id', { required: true });
  ...
  if (body.flexfield_segments == null) {
    errors.push('flexfield_segments is required.');
  }
  const flexfield_segments = validateFlexfieldSegments(body.flexfield_segments, errors);

  rejectForbiddenFields(errors, body, ['assignment_id', 'combined_account_code']);
...
export function validateUpdateCostingAllocationBody(body = {}, _req) {
  const errors = [];
  ...
  rejectForbiddenFields(errors, body, [
    'assignment_id',
    'assignment_number',
...
export function validateDeleteCostingAllocationBody(_body = {}, _query = {}) {
  return {};
}
