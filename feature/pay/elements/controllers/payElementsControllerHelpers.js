...
import { firstValidationMessage } from '../validations/payElements.validation.js';

export const ROUTE_TAG = 'payElements';
export const FALLBACK_ERROR = 'Unable to process pay element. Please try again.';
...
export function sendNotFoundError(res, message = 'Element not found') {
...
export function sendSystemError(res, err) {
  if (err instanceof DatabaseError) {
    const message =
      err.userMessage ||
      resolvePayElementsUserMessage(err.message, err.oracleError) ||
      FALLBACK_ERROR;
 ...
 */
export async function withPayElementErrorHandling(res, work) {
  try {
    return await work();
  } catch (err) {
