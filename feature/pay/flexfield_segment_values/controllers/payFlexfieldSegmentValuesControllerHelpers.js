...
import { firstValidationMessage } from '../validations/payFlexfieldSegmentValues.validation.js';

export const ROUTE_TAG = 'payFlexfieldSegmentValues';
export const FALLBACK_ERROR = 'Unable to process flexfield segment value. Please try again.';
...
export function sendNotFoundError(res, message = 'Segment value not found') {
...
export function sendSystemError(res, err) {
  if (err instanceof DatabaseError) {
    const message =
      err.userMessage ||
      resolveFlexfieldSegmentValueUserMessage(err.message, err.oracleError) ||
      FALLBACK_ERROR;
...
export async function withFlexfieldSegmentValueErrorHandling(res, work) {
  try {
    return await work();
  } catch (err) {
